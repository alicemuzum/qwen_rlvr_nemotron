...
def _lr_from_matches(
    ...
    left_run = (
        max(all_left_runs, key=lambda t: len(t[0])) if all_left_runs else ([], None)
    )
    right_run = (
        max(all_right_runs, key=lambda t: len(t[0])) if all_right_runs else ([], None)
    )
...
def reasoning_bit_manipulation(problem: Problem) -> Optional[str]:
    ...
    trunc_right = (
        f"Truncated right: {_format_list(list(reversed(right_run)), with_count=True)}"
    )
        ...
        for i in range(N_BITS):
            if preferred[i] == "pending":
                if right_is_binary or right_is_unary:
                    preferred[i] = (
                        _extrap_from(right_run, i, right_start_final, "right") or "?"
                    )
                else:
                    preferred[i] = "?"
